...
stu_data = readmatrix('stu_annonymised');
...
for a = 1:length(alpha_trans)
    ...
    for s = 1:num_res
        ...
        for i = 1:length(stu_data)
            ...
            asym_idx = [];
            for h = 1:num_nodes
                inc_period{1,h} = ceil(lognrnd(1.3, 0.89, 1, mat_s(h)));
...
%                 everyone else to them and from them to everyone else (their column and row respectively)
    ...
    total_ego_trans_stu_re = reshape(total_ego_trans_stu, [num_res 1]);
    ...
    total_num_untraced_stu_re = reshape(total_num_untraced_stu, [num_res 1]);
